...
class Game extends Basegame {
    ...
    draw() {
        ...
        Gamestate.context.fillRect(this.camera.calculatePos(new Point(0, 0)).x, this.camera.calculatePos(new Point(0, 0)).y, Gamestate.canvas.width, Gamestate.canvas.height);

        Gamestate.context.fillStyle = "white";
        Gamestate.context.beginPath();
        for (let i = 1; i < this.intersections.length; i++) {
            let drawPosition = this.camera.calculatePos(this.intersections[i])
            Gamestate.context.lineTo(drawPosition.x, drawPosition.y)
        }
        Gamestate.context.fill()
        let curdrawplayerpos = this.camera.calculatePos(this.players[0].position)
